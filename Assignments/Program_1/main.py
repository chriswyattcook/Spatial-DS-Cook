...
if __name__ == '__main__':
   ...
   # print(gd.__dict__)    
    ...
    df.add_polygons(['TX','NY','ME','Kenya', 'CO'])
    ...
    gd.draw_polygons()
    while running:
        # commented this out. Gave me a headache
        # gd.draw_polygons()
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for key, value in gd.adjusted_poly_dict.items():
                    for v in value:
                        if(point_inside_polygon(event.pos[0],event.pos[1],v)):
                            # print(key)
                            pygame.draw.polygon(gd.screen, gd.colors.get_random_color(), v, 4)

                            pygame.init()
                            myfont = pygame.font.SysFont("monospace", 15)

                            # render text
                            label = myfont.render(key, 1, (0,0,0))
                            screen.blit(label, (event.pos[0],event.pos[1]))
                            
                            #these lists hold all the (x,y) from the clicked poly
                            all_x = []
                            all_y = []
